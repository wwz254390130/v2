<template>
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      <el-input @keyup.enter.native="handleFilter" style="width: 200px;" class="filter-item" placeholder="角色" v-model="listQuery.title">
      </el-input>
...
<script>
// 数据接口
import { fetchRolesList, updateRoles } from '@/api/roles'
// 按钮动画特效 - 水波纹指令
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchRolesList(this.listQuery).then(res => {
        this.list = res.data
    ...
    // 前往添加页面
    handleToAdd() {
      this.$router.push({ path: '/system/roles/add' })
    },
    // 前往编辑页面
    handleToEdit(row) {
      this.$router.push({ path: '/system/roles/edit/' + row.id })
    },
    // 修改状态操作
    handleCheck(row) {
      const that = this
    ...
    handleDel(row) {
      ...
      that.$confirm('是否确认删除?', '删除提示', {
        ...
        updateRoles(_opts).then(response => {
          that.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
    ...
    handleDownload() {
      ...
      that.$confirm('将导出当前页面所显示的数据, 是否继续?', '导出提示', {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          // 传入三个参数（表头、数据、导出的文件名）
          excel.export_json_to_excel(tHeader, data, 'roles-list-' + (new Date()).valueOf())
