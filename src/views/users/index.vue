...
<script>
// 数据接口
import { fetchUsersList, updateUsers } from '@/api/users'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchUsersList(this.listQuery).then(res => {
        this.list = res.data
    ...
    // 前往添加页面
    handleToAdd() {
      this.$router.push({ path: '/system/users/add' })
    },
    // 前往编辑页面
    handleToEdit(row) {
      this.$router.push({ path: '/system/users/edit/' + row.id })
    },
    // 修改状态操作
    handleCheck(row) {
      const that = this
    ...
    handleDel(row) {
      ...
      that.$confirm('是否确认删除?', '删除提示', {
        ...
        updateUsers(_opts).then(response => {
          that.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
